...
@login_required
def InvoicePdfSetView(request):
    inv_ids = list(map(int, request.GET['ids'].split(',')))
    pdfs = []
    pages = []

    invoice_pdfs(inv_ids, pdfs, request)

    for doc in pdfs:
        for p in doc.pages:
            pages.append(p)

    pdf_file = pdfs[0].copy(pages).write_pdf()
...
def invoice_pdfs(inv_ids, pdfs, request):
    for invoice_id in inv_ids:
        invoice = Invoice.objects.get(invoice_id=invoice_id)
        products = invoice.invoiceproduct_set.annotate(
            amount=ExpressionWrapper(F("quantity") * F("product_number__price"), output_field=FloatField())).all()
        products = products.order_by("product_number")
        total_quantity = products.aggregate(Sum('quantity'))["quantity__sum"]
        total_amount = products.aggregate(Sum('amount'))["amount__sum"]
        html_template = get_template('app/_invoice_pdf.html').render(
            {'invoice': invoice, 'products': products, "total_quantity": total_quantity, "total_amount": total_amount},
            request)
        pdf_file = HTML(string=html_template, base_url=request.build_absolute_uri()).render()
